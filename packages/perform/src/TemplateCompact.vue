    ...
    <div class="row">
      <div class="col-md-9">
        ...
        <template v-if="view == 'data'">
          <h3>Data</h3>
          <p-data :enactment="enactment" @update-enactment="updateEnactment" :options="options" />
        </template>
        <!-- history view -->
      ...
      <div class="col-md-3 d-none d-md-block">
        <template v-if="enactment.protocol.tasks">
          ...
          <div class="d-grid gap-2">
            <button
              ...
              "
              :key="task.name"
              @click="selectTask({ value: task.path })"
            >
              {{ task.caption || task.name }}
            </button>
          </div>
        ...
        <template v-else>
          ...
          <div class="d-grid">
            <button
              v-bind:class="'btn btn-outline-secondary ' + (view == 'task' ? 'active' : '')"
              @click="view = 'task'"
              block
            >
              <font-awesome-icon icon="clipboard-check" /> Task
            </button>
            <label class="my-2" v-if="debug"
              ><input
                type="checkbox"
                :checked="options.debug"
                @click="$emit('change-option', { option: 'debug', value: !options.debug })"
        ...
        <div class="d-grid gap-2 mt-2">
          <button
            :class="'btn btn-outline-secondary ' + (view == 'data' ? 'active' : '')"
            @click="view = 'data'"
          >
            <font-awesome-icon icon="table" /> Data
          </button>
          <button
            :class="'btn btn-outline-secondary ' + (view == 'history' ? 'active' : '')"
            @click="view = 'history'"
          >
    ...
    <div class="d-md-none">
      <nav class="navbar navbar-light fixed-bottom">
        <div class="container-fluid">
          ...
          <ul class="navbar-nav justify-contents-end">
            <!-- right aligned-->
            <li
              @click="view = 'settings'"
